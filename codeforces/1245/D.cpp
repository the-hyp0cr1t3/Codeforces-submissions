...
const int N = 2e3 + 5;
pii a[N];
int n, m, c[N], k[N], inedgewt[N], parent[N];
vector<pii> edges;
vector<int> stations;
vector<bool> vis;
...
auto dist = [&] (const int& p, const int& q) {
    return (abs(a[p].ff-a[q].ff)+abs(a[p].ss-a[q].ss))*(k[p]+k[q]);
};

int primsMST(int root) {
    ...
    while(!s.empty()) {
        ...
        if(!parent[cur])
            stations.pb(cur);
        if(cur and parent[cur] > 0)
            edges.pb(cur, parent[cur]);
        for(int to = 0; to <= n; to++) {
            if(vis[to] or to == cur)
                continue;
            int edgewt = cur and to? dist(cur, to) : c[cur^to];
            if(inedgewt[to] > edgewt) {               // comparing
...
int32_t main() {
    IOS;
    int i, j;
    re(n); rea1(a, n, c, n, k, n);
    vis.rsz(n+1);
    for(i = 0; i <= n; i++) 
        inedgewt[i] = DESPACITO;
    ps(primsMST(0));
    ps(sz(stations), e, stations, e, sz(edges));
    pn(edges);
    return 0;
}