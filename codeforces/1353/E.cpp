...
int solve () {
    int i, n, k, tot = 0, ans = INF; string s;
    re(n, k, s);
    vector<vector<int>> pref(n, vector<int>(2)), suff(n, vector<int>(2));
    for(auto& x: s) 
        tot += (x == '1');
    for(int kk = 0; kk < k; kk++) {
        ...
        for(i = kk; i < n; i+=k) {
            cnt += (s[i] == '1');
            pref[i][0] = (i < k? 0 : pref[i-k][0]) + (s[i] == '1');
            pref[i][1] = (i < k? 0 : min(pref[i-k][0], pref[i-k][1])) + (s[i] == '0');
        }
        i -= k;
        chmin(res, pref[i][0]);
        for(; i >= 0; i-=k) {
            suff[i][0] = (i+k >= n? 0 : suff[i+k][0]) + (s[i] == '1');
            suff[i][1] = (i+k >= n? 0 : min(suff[i+k][0], suff[i+k][1])) + (s[i] == '0');            
        }
        for(i = kk; i < n; i+=k)
            chmin(res, pref[i][1] + suff[i][1] - (s[i] == '0'));
        chmin(ans, res + tot-cnt);
    }
    ps(ans);
    return 0;
}
