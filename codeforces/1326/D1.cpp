...
string longestPalindromeSubstring(const string &s) {
    ...
    for(int i = 0; i < min(N, 2*sz(s)+3); i++) 
        P[i] = 0;
    ...
    //     }
    // }
    // cout << sz(Q)-2 << "aaa" << endl;
    int bestlen = 0, bestpos = -1, par = 0;
    for(int i = 1; i < sz(Q)-1; i++) {
        if(i&1) {
            if(i/2 == P[i]/2 and P[i] > bestlen)
                bestlen = P[i], bestpos = i, par = 0;
            if((sz(Q)-2-i)/2 == P[i]/2 and P[i] > bestlen)
                bestlen = P[i], bestpos = i, par = 1;
        }
        else {
            if(i/2-1 == P[i]/2 and P[i] > bestlen)
                bestlen = P[i], bestpos = i, par = 0;
            if((sz(Q)-2-i)/2 == P[i]/2 and P[i] > bestlen)
                bestlen = P[i], bestpos = i, par = 1;
        }
    ...
    string ss;
    if(!par)
        ss = s.substr(0, bestlen);
    else
        ss = s.substr(sz(s)-bestlen);
    // cout << "ss " << ss << endl;
    return ss;
...
int solve () {
    ...
    rev(rans);
    string work = s.substr(l, r-l+1);
    pr(lans, longestPalindromeSubstring(work), rans);
    ps();
    return 0;
}
