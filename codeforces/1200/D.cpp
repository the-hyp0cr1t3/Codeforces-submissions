...
const int N = 2e3 + 5;
int a[N][N], rowpref[N][N], colpref[N][N], rowprefk[N][N], colprefk[N][N], rowwhite[N], colwhite[N];
int32_t main() {
    IOS;
    int i, j, n, k, ans = 0;
    re(n, k);
    ...
    for(i = 1; i <= n; i++)
        for(j = 1; j <= n; j++)
            colpref[i][j] = colpref[i][j-1] + a[i][j];
            
    for(j = 1; j <= n; j++)
        for(i = 1; i <= n; i++)
            rowpref[i][j] = rowpref[i-1][j] + a[i][j];
            
    for(i = 1; i <= n; i++) 
        for(j = 1; j+k-1 <= n; j++) 
            colprefk[i][j] = !(colpref[i][n] - colpref[i][j+k-1] + colpref[i][j-1]);

    for(j = 1; j <= n; j++) 
        for(i = 1; i+k-1 <= n; i++) 
            rowprefk[i][j] = !(rowpref[n][j] - rowpref[i+k-1][j] + rowpref[i-1][j]);

    for(i = 1; i <= n; i++) 
        for(j = 1; j <= n; j++) 
            rowprefk[i][j] += rowprefk[i][j-1];

    for(j = 1; j <= n; j++) 
        for(i = 1; i <= n; i++) 
            colprefk[i][j] += colprefk[i-1][j];

    for(i = 1; i <= n; i++) {
        rowwhite[i] = rowwhite[i-1] + !(rowpref[n][i]);
        colwhite[i] = colwhite[i-1] + !(colpref[i][n]);
    }

    for(i = 1; i+k-1 <= n; i++) {
        for(j = 1; j+k-1 <= n; j++) {
            int x = rowprefk[i][j+k-1] - rowprefk[i][j-1];
            int y = colprefk[i+k-1][j] - colprefk[i-1][j];
            int remx = rowwhite[n] - rowwhite[j+k-1] + rowwhite[j-1];
            int remy = colwhite[n] - colwhite[i+k-1] + colwhite[i-1];
            chmax(ans, x + y + remx + remy);
        }
    }
