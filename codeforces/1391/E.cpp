...
int solve() {
    ...
    vector<int> d(n), p(n, -1), store(n+1, -1), path;
    vector<pii> pairing;
    pairing.reserve(n/2); path.reserve(n);
    for(i = 0; i < m; i++) {
        int u, v;
        re(u, v);
        u--, v--;
    ...
    Y([&](auto self, int v, int k) -> void {
        d[v] = k;
        if(k > bestd) bestd = k, b = v;
        if(~store[d[v]]) pairing.pb(store[d[v]]+1, v+1), store[d[v]] = -1;
        else store[d[v]] = v;
        for(auto x: g[v])
            if(!d[x]) p[x] = v, self(x, k+1);
    })(0, 1);

    while(~b) path.pb(b+1), b = p[b];
    
    assert(max(sz(pairing)*2, sz(path)) >= (n+1)/2);
    
    if(sz(path) > sz(pairing)*2) ps("PATH"), ps(sz(path), path);
    else ps("PAIRING"), ps(sz(pairing), pairing);
    return 0;
}
