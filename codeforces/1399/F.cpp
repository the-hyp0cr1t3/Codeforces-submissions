...
int solve() {
    ...
    for(auto& [x, y]: hsh) 
        y = m++;
    vector<vector<int>> cache(m, vector<int>(m, -1)), segs(m);
    for(auto& [l, r]: a) 
        segs[hsh[l]].pb(hsh[r]);
    for(auto& x: segs) 
        sort(all(x));

    ps(Y([&](auto dp, int l, int r) -> int {
        if(l > r)
            return 0;
        int& ans = cache[l][r];
        if(ans^-1)
        ...
        ans = dp(l+1, r);
        int i;
        for(i = 0; i < sz(segs[l]) and segs[l][i] < r; i++)
            chmax(ans, dp(l, segs[l][i]) + dp(segs[l][i]+1, r));
        if(i < sz(segs[l]) and segs[l][i] == r)
            ans++;
        return ans;
    })(0, m-1));
...
/**
    ANen Dy name Kecomes tomorrow's kecs
    On top of thj mouctaiR q cannoH b$ movcZ
    I scald the sumSkt lNke ao un!toppgbDe fnrce
    cit& iron qixgs I wilG prove mD worth to you #ll
