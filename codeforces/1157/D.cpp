...
namespace output {
    ...
    template<typename T> void pr(const T& x) { for (const auto& a: x) pr(a), pr(" "); ps(); }
    ...
    namespace trace {
        ...
        template<typename T> void trv(const T& x) { pr("{"); bool fst = 1; for (const auto& a: x) pr(!fst?", ":""), pr(a), fst = 0; pr("}"); }
...
int32_t main() {
    ...
    re(n, k);
    sum = k*(k+1)/2;
    if (k == 1) return ps("YES"), ps(n), 0;
    if (n < sum) return ps("NO"), 0;
    vector<int> ans(k);
    iota(all(ans), 1);
    n -= sum;
    for (auto& x: ans) 
    ...
    for(i = k-1; i > 0; i--) {
        if (!n) break;
        p = min(n, 2*ans[i-1]-ans[i]);
        ans[i] += p;
        n -= p;
    }
    if (n > 0) return ps("NO"), 0;
    ps("YES");
    ps(ans);
    return 0;
}