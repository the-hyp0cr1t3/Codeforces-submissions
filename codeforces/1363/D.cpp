...
int solve () {
    ...
    mx = query(v);
    auto mxi = binarySearch(1, n);

    for(i = 0; i < k and kk == -1; i++) 
        if(subsets[i].count(mxi)) kk = i;

    vector<int> ans(k, mx);
    if(~kk) {
        v.clear();
        for(i = 1; i <= n; i++) 
            if(!subsets[kk].count(i)) v.pb(i);
        ans[kk] = query(v);
    }
