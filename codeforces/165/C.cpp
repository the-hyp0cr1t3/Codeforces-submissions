...
int32_t main() {
    IOS;
    int i, k; string s; ll ans = 0;
    re(k, s); v.pb(-1);
    for(i = 0; i < sz(s); i++) 
        if (s[i] == '1') v.pb(i);
    v.pb(sz(s));
    if (sz(v) < k+2) return ps(0), 0;
    if (k)
        for(int l = 1, r = k; r < sz(v)-1; l++, r++)
            ans += 1ll*(v[l]-v[l-1])*(v[r+1]-v[r]);
    else
        for(int l = 0, r = 1; r < sz(v); l++, r++)
            ans += 1ll*(v[r]-v[l]-1)*(v[r]-v[l])/2;
    ps(ans);
    return 0;
}
