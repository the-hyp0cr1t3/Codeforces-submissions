...
int solve () {
    ...
    auto add = [&] (map<int,int>& c, int kk, map<int,int>& d) {
        c[kk]++;
        int cnt = c[kk]/p;
        while(cnt) {
            c[kk] %= p;
        ...
        chmax(curtop, kk);
        if(c[curtop] == d[curtop]) curtop = 0;
    };

    for(i = 0; i < n; i++) {
        // tr(i, whosbigger(), curtop);
        if(whosbigger())
            add(b, k[i], a);
        else
            add(a, k[i], b);
    ...
    num ans1, ans2;
    for(auto& [kk, v]: a)
        ans1 += expo(p, kk) * num(v);
    for(auto& [kk, v]: b)
        ans2 += expo(p, kk) * num(v);
