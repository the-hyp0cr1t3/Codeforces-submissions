...
const int M = (1<<16);
const int MxA = 58;
int a[N], prime[MxA+1], mask[MxA+1], dp[N][M], ans[N];
pii path[N][M];
...
int32_t main() {
    ...
    for(i = 2; i <= MxA; i++) {
        int x = i;
        for(j = 2; j*j <= x and x > 1; j++) {
            if(x%j) continue;
            while(x % j == 0)
                x /= j;
            mask[i] |= (1<<prime[j]);
        }
        if(x > 1) mask[i] |= (1<<prime[x]);
    }
    ...
    for(i = 1; i <= n; i++) {
        for(m = 0; m < M; m++) {
            ...
            for(j = 1; j <= MxA; j++) {
                ...
                if(dp[i-1][m] + abs(a[i]-j) < dp[i][m|mask[j]]) {
                    dp[i][m|mask[j]] = dp[i-1][m] + abs(a[i]-j);
                    path[i][m|mask[j]] = {m, j};
                }
            }
        }
    ...
    int bestmask = -1, best = INF;
    for(m = 0; m < M; m++)
        chmin2(best, bestmask, dp[n][m], m);

    for(i = n; i; i--) {
        ans[i] = path[i][bestmask].ss;
        bestmask = path[i][bestmask].ff;
    }
