...
const int N = 100 + 5;
int n[2], k[2], cache[N][N][N][2];
num dp (int n1, int n2, int len, int cur) {
    if (n1 > n[0] or n2 > n[1] or len > k[cur])
        return num(0);
    if (n1 == n[0] and n2 == n[1])
        return num(1);
    int &ans = cache[n1][n2][len][cur];
    if (ans != -1)
        return num(ans);
    num res = dp(n1 + (cur^1), n2 + (cur), len+1, cur) + dp(n1 + (cur), n2 + (cur^1), 1, cur^1);
    ans = int(res);
    return res;
}
