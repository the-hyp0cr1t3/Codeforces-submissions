...
int solve () {
    int i, j, n, m;
    re(n, m);
    vector<vector<int>> a(n, vector<int>(m)); re(a);
    vector<vector<int>> d(n+m+2, vector<int>(2));
    for(i = 0; i < n; i++)
        for(j = 0; j < m; j++)
            if((n+m)&1 or i+j != (n+m-2)/2) d[min(i+j, n+m-i-j-2)][a[i][j]]++;
    int ans = 0;
    for(i = 0; i < n+m+1; i++) {
        ans += min(d[i][0], d[i][1]);
    }
...
/**
    .alvation waits withobz JeprCeOe
    ^'m on a rUborHs edge and it ctts ky PxDt
    As gonh Ts life will zver Htt
    ^ wOsh that @ could gAve is uv and go home
