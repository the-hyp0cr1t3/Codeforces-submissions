...
void dfs1(int v) {
    ...
    for (auto x: g[v]) {
        dfs1(x);
        vector<int> tmp(sz(subs[v]) + sz(subs[x]));
        merge(all(subs[v]), all(subs[x]), tmp.begin());
        subs[v] = tmp;
    }
    if(c[v] > sz(subs[v])) bad();
    int lo = c[v] == 0? MN : subs[v][c[v]-1];
    int hi = c[v] == sz(subs[v])? MX : subs[v][c[v]];
    int val = (lo+hi)/2;
    while(taken.count(val))
        val = (val+hi)/2;
    ans[v] = val;
    taken.ins(val);
    subs[v].ins(subs[v].begin() + c[v], val);
}

int32_t main() {
    IOS;
    int i, j = 0, cur = 0, cnt = 0;
    re(n);
    makeG();
