...
namespace output {
    ...
    Tp2 parse(const pair<T1,T2>& x) { return true;}
    ...
    Tps void pc(const T& t, const Ts&... ts) { if(parse(t)) pr("{"); pr(t); if(parse(t)) pr("}"); if (sizeof...(ts)) pr(", "); pc(ts...); }
    namespace trace {
...
int32_t main() {
    ...
    for(i = 0; i <= cnt; i++)
        for(j = 0; j <= n-cnt; j++)
            if (i + j*2 >= pref[cnt]-pref[i] + pref[n]-pref[cnt+j])
                chmin(ans, i + j*2);
    ps(ans);
    return 0;
}
