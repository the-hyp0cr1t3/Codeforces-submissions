...
void dijkstra(int root) {
    ...
    while(!q.empty()) {
        ...
        auto Dfs = [&](auto& dfs, int u, int curdis) {
            vis[u] = 1;
            if(d[u] > d[v] + c[v]) {
                q.erase({d[u], u});
                d[u] = d[v] + c[v];
            ...
            for(int to = 0; to < n; to++)
                if(g[u][to] and !vis[to] and curdis+g[u][to] <= t[v]) 
                    dfs(dfs, to, curdis+g[u][to]);
        };
        Dfs(Dfs, v, 0);
    }
}
...
int32_t main() {
    ...
    makeG();
    dijkstra(x);
    if(d[y] == DESPACITO) return ps(-1), 0;
    ps(d[y]);
    return 0;
}