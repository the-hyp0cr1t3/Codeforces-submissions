...
int32_t main() {
    IOS;
    int i, n, edgecnt = 0;
    re(n); vector<pii> a(n+1);
    for(i = 1; i <= n; i++) 
        re(a[i]);
    vector<pii> v; v.reserve(2*n);
    for(i = 1; i <= n; i++) 
        v.pb(a[i].ff, -i), v.pb(a[i].ss, i);
    sort(all(v));
    build(n+1);
    set<pii> active;
    for(auto& [x, id]: v) {
        if(id < 0) {
            ...
            active.ins(mp(a[id].ss, id));
        }
        else
            active.erase(mp(x, id));
    }
    int pp = getpar(1);
    for(i = 1; i <= n; i++) 
        if(pp^getpar(i)) return ps("NO"), 0;
    ps("YES");
    return 0;
}