...
const int N = 3e6 + 5;
vector<int> moves[N], ans[N];
int32_t main() {
    IOS;
    int i, j, n, k; string s;
    re(n, k, s);
    int mx = 0, mn = 1, cntl = 0;
    for(i = 0; i < n; i++) {
        if(s[i] == 'R') continue;
        mx += i - cntl;
        cntl++;
    }
    if(k > mx) return ps(-1), 0;
    for(int t = 1, ok = 0; !ok; t++) {
        ok = 1;
        for(i = 1; i < n; i++) {
    ...
    mn--;
    if(k < mn) return ps(-1), 0;
    int curt = 1;
    for(i = 1; i <= mn; i++) {
        bool vis = 0;
        int rem = mn-i+sz(moves[i]);
        for(j = 0; j < sz(moves[i]); j++) {
            if(k-curt+1 > rem) break;
            ans[curt].pb(moves[i][j]);
            rem--;
            vis = 1;
        }
        if(vis)
