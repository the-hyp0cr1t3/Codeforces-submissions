...
const int N = 2e5 + 5;
int n, m, k, deg[N], d[N], p[N];
vector<int> g[N], vis, cycle, isactive;
set<pii> active;
...
void makeG() {
    ...
    for(int i = 0; i < n; i++) 
        active.ins(mp(deg[i], i));
    vis.rsz(n);
    isactive.assign(n, 1);
    cycle.reserve(k);
}

bool dfs(int v, int dd) {
    d[v] = dd;
    vis[v] = 1;
    for(auto x: g[v]) {
        if(vis[x] and d[v]-d[x]+1 <= k and x^p[v]) {
            cycle.pb(x+1);
            while(v^x)
                cycle.pb(v+1), v = p[v];
            ps(sz(cycle), e, cycle);
...
int32_t main() {
    ...
    makeG();
    vector<int> indset;
    indset.reserve(k/2+1);
    while(!active.empty()) {
        auto [dig, v] = *active.begin();
        isactive[v] = 0;
        ...
        for(auto& u: g[v]) {
            ...
            for(auto& x: g[u]) {
                ...
                active.erase(mp(deg[x], x));
                deg[x]--;
                active.ins(mp(deg[x], x));
            }
            isactive[u] = 0;
            active.erase(mp(deg[u], u));
        }
        indset.pb(v+1);
        if(sz(indset) == (k+1)/2) {
            ps(1, e, indset);
