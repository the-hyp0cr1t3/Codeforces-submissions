...
void bfs(int root) {
    ...
    while (!q.empty()) {
        ...
        for (auto [nxt, w] : g[cur]) {
            if (d[root][cur] + w < d[root][nxt]) {
                d[root][nxt] = d[root][cur] + w;
                q.push(nxt);
            }
        }
    }
...
int32_t main() {
    ...
    for(i = 0; i < n; i++) 
        for(j = 0; j < n; j++) 
            re(a[i][j]), d[i][j] = DESPACITO;

    for(i = 0; i < n; i++) {
        if(a[i][i]) return ps("NO"), 0;
        d[i][i] = 0;
    }
    for(i = 0; i < n; i++) {
        for(j = 0; j < i; j++) {
            if(a[i][j]^a[j][i] or !a[i][j]) return ps("NO"), 0;
            else ss.pb(a[i][j], mp(i, j));
        }
    }
    ...
    for(auto& [dist, p]: ss) {
        if(merge(p.ff, p.ss)) {
            cnt++;
            g[p.ff].pb(p.ss, a[p.ff][p.ss]);
            g[p.ss].pb(p.ff, a[p.ff][p.ss]);
        }
    }
