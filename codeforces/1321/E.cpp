...
struct equipment {
    int stats, cost;
    equipment() : stats(0), cost(0) {}
    equipment(int _s, int _c) : stats(_s), cost(_c){}
};
struct Monsters {
    int x, y, z;
    Monsters() : x(0), y(0), z(0) {}
    Monsters(int _x, int _y, int _z) : x(_x), y(_y), z(_z){}
};
...
struct SegTree {
    ...
    void build(int node, int L, int R, vector<equipment>& a) {
        if(L == R) {
            st[node].val = -a[L-1].cost;       // .
            return;
        }
        int M = (L+R)/2;
        build(node*2, L, M, a);
...
int32_t main() {
    IOS;
    int i, j = 0, n, m, p, ans = -DESPACITO;
    re(n, m, p);
    vector<equipment> a(n), b(m);
    vector<Monsters> monsters(p);
    for(i = 0; i < n; i++) 
        re(a[i].stats, a[i].cost);
    for(i = 0; i < m; i++) 
        re(b[i].stats, b[i].cost);
    for(i = 0; i < p; i++) 
        re(monsters[i].x, monsters[i].y, monsters[i].z);

    sort(all(a), [](const equipment& A, const equipment& B) { 
        return A.stats < B.stats; 
    });    
    sort(all(b), [](const equipment& A, const equipment& B) { 
        return A.stats < B.stats; 
    });
    sort(all(monsters), [](const Monsters& A, const Monsters& B) { 
        return A.x < B.x; 
    });    
    ...
    auto check = [&] (int x, int val) {
        return b[x].stats > val;
    };
    
    auto binarySearch = [&] (int low, int high, int val) {
        while(low <= high) {
            int mid = (low + high) / 2;
            if(check(mid, val))
                high = mid - 1;
    ...
    for(auto& [stat, cost]: a) {
        while(j < p and monsters[j].x < stat) {
            int idx = binarySearch(0, m-1, monsters[j].y) + 1;
            st.update(idx, m, monsters[j].z);
            j++;
        }
        int res = -cost + st.query();
        chmax(ans, res);
    }
