...
int32_t main() {
    IOS;
    int i, n, j, m, ans = -1, best;
    re(n, m); a.rsz(n); segs.rsz(m); re(a, segs);
    ...
    for(i = 0; i < n; i++) {
        for(j = 0; j < m; j++) {
            if(active[j]) {
                if(i+1 >= segs[j].ff and i+1 <= segs[j].ss)
                    st.update(segs[j].ff, segs[j].ss, 1), active[j] = 0;
            }
            else {
                if(!(i+1 >= segs[j].ff and i+1 <= segs[j].ss))
                    st.update(segs[j].ff, segs[j].ss, -1), active[j] = 1;                
            }
        }
        auto x = st.query();
        int res = x.mx - x.mn;
        chmax2(ans, best, res, i);
    }
    vector<int> chosen;
    for(j = 0; j < m; j++) {
        if(best+1 >= segs[j].ff and best+1 <= segs[j].ss) continue;
        chosen.pb(j+1);
    }
    ps(ans, e, sz(chosen), e, chosen);
    return 0;
}