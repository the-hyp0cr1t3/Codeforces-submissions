...
struct lmao {
    int pos, cost, idx;
    lmao(int p, int c, int i) : pos(p), cost(c), idx(i) {}
};
auto cmp1 = [] (const lmao A, const lmao B) {
    if (A.pos == B.pos)
        return A.idx < B.idx;
    return A.pos < B.pos;
};
auto cmp2 = [] (const lmao A, const lmao B) {
    if (A.cost == B.cost)
        return A.idx < B.idx;
    return A.cost < B.cost;
};
set <lmao, decltype(cmp1)> points(cmp1);
priority_queue <lmao, vector<lmao>, decltype(cmp2)> pq(cmp2);
int32_t main() {
    IOS;
    int i, n, cur = 0, ans = 0;
    re(n); rea(a, n); rea(t, n);
    for(i = 0; i < n; i++)
        points.ins(lmao(a[i], t[i], i));

    for (auto p: points) {
        while(!pq.empty() and cur < p.pos) {
            auto x = pq.top();
            pq.pop();
            ans += x.cost * (cur-x.pos);
            cur++;
        }
        pq.push(p);
        chmax(cur, p.pos);
