...
struct Data {
    int sum;
    vector<int> cnt = vector<int>(B, 0);
    Data() : sum(0) {}

    void init(int idx) {
        for(int i = 0; i < B; i++)
            this->cnt[i] = !!(a[idx-1] & (1<<i));
        this->sum = a[idx-1];
    }

    void merge(const Data& l, const Data& r) {
        for(int i = 0; i < B; i++)
            this->cnt[i] = l.cnt[i] + r.cnt[i];
        this->sum = l.sum + r.sum;
    }

    void lazymerge(int len, int val) {
        for(int i = 0; i < B; i++) {
            if(val & (1<<i)) {
                this->sum -= this->cnt[i] * (1<<i);
                this->cnt[i] = len - this->cnt[i];
                this->sum += this->cnt[i] * (1<<i);
            }
        }
    }
...
struct SegTree {
    ...
    void propagate(int node, int L, int R) {
        ...
        st[node].lazymerge(R-L+1, lazy[node]);
        cLazy[node] = 0;
        lazy[node] = 0;
    }
