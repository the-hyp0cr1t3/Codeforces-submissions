...
int32_t main() {
    ...
    for(i = 1, j = 1; i <= n; i = j) {
        ...
        int mcnt = j-i, lcnt = i-1, rcnt = n-j+1;
        int lsum = (i-1)*(a[i]-1) - pref[i-1];
        int rsum = pref[n]-pref[j-1] - (n-j+1)*(a[i]+1);
        int rem = k - mcnt;
        if(rem <= 0) chmin(ans, 0ll);
        if(rem <= lcnt)
            chmin(ans, lsum + rem);
        if(rem <= rcnt)
            chmin(ans, rsum + rem);
        chmin(ans, lsum + rsum + rem);
    }
    chmax(ans, 0ll);
    ps(ans);
    return 0;
