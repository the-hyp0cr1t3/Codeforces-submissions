...
int32_t main() {
    ...
    for(i = 1; i <= n; i++) 
        for(j = 1; j <= m; j++) 
            dp[i][j][L] = a[i][j]? 0 : dp[i][j-1][L] + 1;
    for(i = 1; i <= n; i++) 
        for(j = m; j; j--) 
            dp[i][j][R] = a[i][j]? 0 : dp[i][j+1][R] + 1;
    for(j = 1; j <= m; j++) 
        for(i = 1; i <= n; i++) 
            dp[i][j][U] = a[i][j]? 0 : dp[i-1][j][U] + 1;
    for(j = 1; j <= m; j++) 
        for(i = n; i; i--) 
            dp[i][j][D] = a[i][j]? 0 : dp[i+1][j][D] + 1;

    vector<point> ans;
    for(i = 1; i <= n; i++) {
        for(j = 1; j <= m; j++) {
            if (a[i][j]) continue;
            int x = min(dp[i][j-1][L], dp[i][j+1][R], dp[i-1][j][U], dp[i+1][j][D]);
            if (!x) continue;
            ans.pb(i, j, x);
            for(int k = 0; k <= x; k++) 
                ok[min(n, i+k)][j] = ok[max(1, i-k)][j] = ok[i][min(m, j+k)] = ok[i][max(1, j-k)] = 1;
        }
    }
