...
int32_t main() {
    ...
    for(i = 0; i < n; i++) 
        if(!in[i]) topsort.pb(i);
    for(i = 0; i < sz(topsort); i++) {
        int cur = topsort[i];
        for(auto& nxt: succ[cur]) 
            if(!--in[nxt]) topsort.pb(nxt);
    }
    if(sz(topsort) < n)
        return ps(-1), 0;
    vector<int> mnsucc(n), mnpred(n);
    iota(all(mnsucc), 0);
    iota(all(mnpred), 0);
    for(auto& cur: topsort) 
        for(auto& prv: pred[cur]) 
            chmin(mnpred[cur], mnpred[prv]);
    for(i = n-1; ~i; i--) {
        int cur = topsort[i];
        for(auto& nxt: succ[cur]) 
            chmin(mnsucc[cur], mnsucc[nxt]);
    }
    string ans(n, 'E');
    for(i = 0; i < n; i++) 
        if(mnpred[i] == i and mnsucc[i] == i) 
            cnt++, ans[i] = 'A';
    ps(cnt, e, ans);
    return 0;
}