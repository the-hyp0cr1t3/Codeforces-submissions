...
int dfs(int curx, int cury) {
    vis[curx][cury] = 1;
    if(a[curx][cury] == '.') return 0;
    int ret = 1;
    for(int i = 0; i < 4; i++) {
        int x = curx + dx[i];
        int y = cury + dy[i];
        if(x >= 0 and x < n and y >= 0 and y < m and !vis[x][y] and a[x][y] == '#')
            ret += dfs(x, y);
    }
    return ret;
}
...
int solve () {
    ...
    for(i = 0; i < n; i++) 
        for(j = 0; j < m; j++) 
            if(!vis[i][j] and dfs(i, j)) ans++;
    for(i = 0; i < n; i++) {
        bool bad = 0, seen = 0;
        for(j = 0; j < m; j++) {
            if(bad and a[i][j] == '#') return ps(-1), 0;
            if(seen and a[i][j] == '.') bad = 1;
            if(a[i][j] == '#') seen = 1;
        }
    }
    for(j = 0; j < m; j++) {
    ...
    if(sz(whitecol) or sz(whiterow)) return ps(-1), 0;
    ps(ans);
    return 0;
}
