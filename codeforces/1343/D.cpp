...
int solve () {
    ...
    for(i = 0, j = n-1; i < j; i++, j--) {
        cnt[a[i]+a[j]]++;
        mns.pb(min(a[i], a[j]));
        mxs.pb(k+max(a[i], a[j]));
    }
    sort(all(mns));
    sort(all(mxs));
    int r = mxs[0], l = mns.back();
    assert(l < r);
    for(auto& [sum, cntt]: cnt) {
        int x = int(lb(all(mns), sum) - mns.begin());
        x = n/2 - x;
        int y = int(lb(all(mxs), sum) - mxs.begin());
        chmin(ans, n/2-cntt-x-y+2*(x+y));
    }
    chmin(ans, n/2);
    ps(ans);
    return 0;
