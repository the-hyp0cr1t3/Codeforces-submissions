...
const int N = 2e5 + 5;
int n, m, arrive[N], depart[N], p[N];
vector<pii> g[N];
vector<int> wait[N], nxt[N];

int waitidx(int v, int t) {
    int low = 0, high = sz(wait[v])-1;
    while(low <= high) {
        int mid = (low + high) / 2;
        if(wait[v][mid] <= t)
            low = mid + 1;
    ...
    return ~high and wait[v][high] == t? high : -1;
}

int depnxt(int v, int t) {
    int x = waitidx(v, t);
    return ~x? nxt[v][x] : t;
}
...
void dijkstra(int root) {
    ...
    set<pii> q;
    for(i = 0; i < n; i++) 
        depart[i] = arrive[i] = INF;
    arrive[root] = 0;
    depart[root] = depnxt(0, 0);
    q.ins({arrive[root], root});
    while(!q.empty()) {
        auto v = *q.begin();
        q.erase(q.begin());
        for(auto& x: g[v.ss]) {
            if(arrive[x.ff] > depart[v.ss] + x.ss) {
                q.erase({arrive[x.ff], x.ff});
                arrive[x.ff] = depart[v.ss] + x.ss;
                depart[x.ff] = depnxt(x.ff, arrive[x.ff]);
                q.ins({arrive[x.ff], x.ff});
            }
        }
    }
}
...
int32_t main() {
    ...
    makeG();
    dijkstra(0);
    if(arrive[n-1] == INF) arrive[n-1] = -1;
    ps(arrive[n-1]);
    return 0;
}