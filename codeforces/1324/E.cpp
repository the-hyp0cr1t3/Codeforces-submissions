...
const int N = 2000 + 5;
int a[N], dp[N][N];
// dp[i'th sleep][ending at j'th hour]

int32_t main() {
    IOS;
    int i, j, n, h, l, r, ans = 0;
    re(n, h, l, r); rea1(a, n);
    for(i = 0; i < N; i++) 
        for(j = 0; j < N; j++) 
            dp[i][j] = -1;
    ...
    for(i = 1; i <= n; i++) {
        for(j = 0; j < h; j++) {
            ...
            int x = j + a[i];
            x %= h;
            chmax(dp[i][x], dp[i-1][j] + (x >= l and x <= r));
            x = j + a[i] - 1;
            x %= h;
            chmax(dp[i][x], dp[i-1][j] + (x >= l and x <= r));
        }
    ...
    // for(i = 0; i <= n; i++) 
    //     for(j = 0; j < h; j++) 
    //         cout << (dp[i][j] > -1? dp[i][j] : 9) << " \n"[j == h-1];
        
    for(i = 0; i < h; i++)
        if (dp[n][i] > -1) chmax(ans, dp[n][i]);
    ps(ans);
    return 0;
}
