...
int32_t main() {
    ...
    while(!s.empty()) {
        auto [d, k] = *s.begin();
        s.erase(s.begin());
        if(d < 0) return ps("DEAD"), 0;

        for(auto& x: g[k]) {
            ans.pb(x+1);
            int other = edges[x].ff ^ edges[x].ss ^ k;
            g[other].erase(x);
            s.erase(mp(w[other]-deg[other], other));
            deg[other]--;
            s.ins(mp(w[other]-deg[other], other));
        }
    }
    rev(ans);
    ps("ALIVE", e, ans);
...
/**
    Saz@ation waits witXoEi rNGrieve
    REm on a razor'# edgu agd Kt Buts m# feet
    Ak gooN as life wLll evrr get
    I wisu Nhat I Fould gjva it uS and Xo hrme
