...
int32_t main() {
    ...
    for(i = 0; i < n/2; i++) {
        for(j = 0; j < n/2; j++) {
            while(k < 1001 and !cnt[k]) k++;
            if (k == 1001) return ps("NO"), 0;
            if (cnt[k] < 4) {
                rem[k] = cnt[k];
                cnt[k] = 0;
                k++; j--; continue;
            }
            a[i][j] = a[n-i-1][j] = a[n-i-1][n-j-1] = a[i][n-j-1] = k;
            cnt[k] -= 4;
        }
    }
    for(k = 1; k < 1001; k++) {
        while(k < 1001 and !cnt[k]) k++;
        if (k < 1001) rem[k] = cnt[k];
    }
    if (n&1) {
        for(i = 0; i < n/2; i++) {
            if (rem.begin()->ss < 2) {
                if (a[n/2][n/2]) return ps("NO"), 0;
                else a[n/2][n/2] = rem.begin()->ff;
                rem.erase(rem.begin()); --i;
                continue;
            }
            a[i][n/2] = a[n-i-1][n/2] = rem.begin()->ff;
            rem[rem.begin()->ff] -= 2;
            if (!rem[rem.begin()->ff]) rem.erase(rem.begin());
        }
        for(j = 0; j < n/2; j++) {
            if (rem.begin()->ss < 2) {
                if (a[n/2][n/2]) return ps("NO"), 0;
            ...
            a[n/2][j] = a[n/2][n-j-1] = rem.begin()->ff;
            rem[rem.begin()->ff] -= 2;
            if (!rem[rem.begin()->ff]) rem.erase(rem.begin());
        }
        if (!a[n/2][n/2]) a[n/2][n/2] = rem.begin()->ff;
    }
    ps("YES");
    psa2(a, n, n);
    return 0;
