...
int32_t main() {
    ...
    auto check = [&] (double x) {
        ...
        for(int z = 0; z < n; z++) 
            v[z] = a[z]-x;
        return double(max(kadanes(v), kadanes(v)));
    };
    
    auto ternarySearch = [&] (double low, double high) {
        double eps = 1e-9;
        for(i = 0; i < 100; i++) {
            double m1 = (2.0*low + high) / 3;
            double m2 = (low + 2.0*high) / 3;
            if(check(m1) > check(m2))
                low = m1;
            else
                high = m2;
        }
        return double(check(low));
    };
    
    cout << fixed << setprecision(7) << ternarySearch(-1e4, 1e4);
    return 0;
}