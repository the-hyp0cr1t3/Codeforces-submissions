...
void dfs(int x, int y, vector<vector<int>>& vis, vector<string>& a) {
    vis[x][y] = 1;
    if(x and !vis[x-1][y] and a[x-1][y] == '1')
        dfs(x-1, y, vis, a);
    if(y and !vis[x][y-1] and a[x][y-1] == '1')
        dfs(x, y-1, vis, a);
}
...
int solve () {
    int i, j,n;
    re(n); vector<string> a(n); re(a);
    vector<vector<int>> vis(n, vector<int>(n));
    for(i = 0; i < n; i++) {
        if(!vis[n-1][i] and a[n-1][i] == '1') dfs(n-1, i, vis, a);
        if(!vis[i][n-1] and a[i][n-1] == '1') dfs(i, n-1, vis, a);
    }
    for(i = 0; i < n; i++) 
        for(j = 0; j < n; j++) 
            if(a[i][j] == '1' and !vis[i][j]) return ps("NO"), 0;
    ps("YES");
    return 0;
}
