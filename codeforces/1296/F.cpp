...
bool dfs1(int v, int last, int w) {
    vis[v] = 1;
    if (v == last) return true;
    for (auto x: g[v])
        if (!vis[x.ff] and dfs1(x.ff, last, w))
            return ans[x.ss] = w, true;
    return false;
}

int dfs2(int v, int last) {
    vis[v] = 1;
    if (v == last) return INF;
    int res = INF;
    for (auto x: g[v])
        if (!vis[x.ff]) {
            int y = dfs2(x.ff, last);
            if(y) chmin(res, y, ans[x.ss]);
        }
...
int32_t main() {
    ...
    for(i = 0; i < m; i++) {
        ...
        re(u, v, t);
        u--, v--;
        ranges.pb(mp(t, mp(u, v)));
    }
    sort(all(ranges));
    for (auto& x: ranges) 
        dfs1(x.ss.ff, x.ss.ss, x.ff), vis.assign(n, 0);
    for (auto& x: ranges) 
        ok &= x.ff == dfs2(x.ss.ff, x.ss.ss), vis.assign(n, 0);
