...
int solve () {
    int i, j, k, a, b, c, A, B, C, ans = INF, diffc, cc;
    re(a, b, c);
    auto update = [&] (int aa) {
        int x = abs(a-aa) + abs(b-j) + diffc;
        if (x < ans) ans = x, A = aa, B = j, C = cc;
    };
    for(j = 1; j < N; j++) {
        if (c <= j)
            diffc = j-c, cc = j;
        else {
            if (c - c/j*j > (c+j-1)/j*j - c or c/j*j < j)
                diffc = (c+j-1)/j*j - c, cc = (c+j-1)/j*j;
            else
                diffc = c - c/j*j, cc = c/j*j;
        }
        for (auto& aa: divisors[j]) 
            update(aa);
    }
