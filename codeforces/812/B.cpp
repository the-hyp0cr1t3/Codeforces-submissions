...
const int N = 15 + 5;
const int M = 100 + 5;
int n, m, lastl[N], lastr[N], dp[N][2][2];
...
int32_t main() {
    ...
    for(i = n; i; i--) {
        string s; re(s);
        auto p = s.rfind('1');
        lastl[i] = (int)(p == string::npos? 0 : p);
        auto q = s.find('1');
        lastr[i] = (int)(q == string::npos? m+1 : q);
        if (highest == -1 and lastl[i]) highest = i;
    }
    if (highest == -1) return ps(0), 0;

    dp[0][1][1] = dp[0][0][1] = INF;
    for(i = 1; i < highest; i++) {
        dp[i][0][0] = 1 + min(dp[i-1][0][0], dp[i-1][1][0]) + lastl[i]*2;
        dp[i][0][1] = 1 + min(dp[i-1][0][0], dp[i-1][1][0]) + m+1;
        dp[i][1][1] = 1 + min(dp[i-1][1][1], dp[i-1][0][1]) + (m+1-lastr[i])*2;
        dp[i][1][0] = 1 + min(dp[i-1][1][1], dp[i-1][0][1]) + m+1;
    }
    dp[highest][0][0] = min(dp[i-1][0][0], dp[i-1][1][0]) + lastl[highest];
    dp[highest][1][0] = min(dp[i-1][1][1], dp[i-1][0][1]) + (m+1-lastr[highest]);
    ps(min(dp[highest][0][0], dp[highest][1][0]));
    return 0;
}