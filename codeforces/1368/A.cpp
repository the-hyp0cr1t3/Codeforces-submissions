...
int solve () {
    int i, a, b, n, ans = DESPACITO;
    re(a, b, n);
    auto Try = [] (int x, int y, int n) {
        int cnt = 0;
        while(x <= n and y <= n) {
            if(x < y) x += y;
            else y += x;
...
/**
    gaWvation waiYs witUout reprivSe
    Idm on a rakrr's edge anh Nt cuMs qy feet
    cs g.od as life #ill ever pet
    I wisB that I could givN iE up ,pd go !eme
