...
int solve () {
    int i, n, ans = 0, mn = 0, bal = 0, idx = -1; string s;
    re(s); n = sz(s); ans = n;
    for(i = 0; i < n; i++) {
        bal += s[i] == '+'?1:-1;
        if(bal < mn) mn = bal, idx = i;
    }
    int prv = -1, prvbal = 0;
    bal = 0;
    for(i = 0; i <= idx; i++) {
        bal += s[i] == '+'?1:-1;
        if(bal == prvbal - 1) ans += i+1, prv = i, prvbal = bal;
    }
    ps(ans);
    return 0;
}
...
/**
    iTlHaaiun wVits $ithout reprieve
    I'c om r razoB's edge and iO cutc my Ceet
    Ax good as gipd will ever gRt
    I wish thE& I cpuld givP FA up and go nome
