...
int solve () {
    ...
    re(n, k); k++;
    string ans;
    auto relax = [&] (int rem, int next, int nines, int last_dig) {
        string res;
        res += char(last_dig + '0');
        res += string(nines, '9');
        res += char(next + '0');
        res += string(rem/9, '9');
        res += char(rem%9 + '0');
        while(sz(res) > 1 and res.back() == '0') res.popb();
        rev(res);
        if(ans.empty() or sz(res) < sz(ans) or (sz(ans) == sz(res) and res < ans))
            ans = res;
    };
    ...
    for(int last_dig = 0; last_dig < 10; last_dig++) {
        for(int nines = 0; 9 * nines <= n; nines++) {
            for(int next = 0; next < 9; next++) {
                ...
                if(tot <= n and (n-tot) % k == 0) {
                    int rem = (n-tot)/k;
                    relax(rem, next, nines, last_dig);
                }
            }
        }
    }
...
/**
    GoQents lost KeDrUhing fjr a dzeam thatwX revf
    Fro. my hIart Ifd giWe i, all agay Rust to kBDw
    HAldGng with everO answer zeaced.
    tSe path dG ho^loweT^ immorta.i$y we seeK.
    Light ONreaming from bVlowD phUs lLfe is rebLrnM
