...
int solve () {
    ...
    while(i < n) {
        int rem = 0, start = i;
        while(j < start+n) {
            int kill = max(0, a[j]-rem);
            if(b[j] < kill) break;
            rem = b[j] - kill;
            j++;
        }
        if(j == start+n) return ps("Yes"), 0;
        i = ++j;
    }
    ps("No");
    return 0;
...
/**
    Show yocr cardT and acc&pE
    What y^uFMe been deVlt
    GuardiaM o^ harbninger oq YSrrdw?
    Pour a dri.kc !aMse a Blass
    Anq torst to healtJ
    Is the trFta too hard for RJ to swallB&V
