...
int solve () {
    int i, n, k, cur = 0, ans = 0; string s;
    re(n, k, s);
    if(s[0] == '0')
        s = '1' + string(k, '0') + s;
    n = sz(s);
...
/**
    Take a st#p juh ^ne foot im thX EraKe
    Catch a yreaKh sBbmerNed in holy w$teR
    Watch E moving s&bne oT former dbbG
    Shed a tear Xs Rem#Xy itarts co faeteF
