...
void makeG() {
    for(int i = 0; i < m; i++) {
        ...
        re(u, v, w);
        u--, v--;
        g[u].ins(mp(v, mp(w, i+1)));
        g[v].ins(mp(u, mp(w, i+1)));
    }
}
...
void dijkstra(int root) {
    ...
    while(!q.empty()) {
        ...
        q.erase(q.begin());
        for(auto& x: g[v.ss]) {
            bool equal = (d[x.ff] == (d[v.ss] + x.ss.ff)) and (d[v.ss] < d[x.ff]);
            if(equal or (d[x.ff] > d[v.ss] + x.ss.ff)) {
                q.erase({d[x.ff], x.ff});
                d[x.ff] = d[v.ss] + x.ss.ff;
                q.ins({d[x.ff], x.ff});
                p[x.ff] = v.ss;
            }
        }
