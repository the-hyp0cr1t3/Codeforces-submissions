...
int the_real_main() {
    ...
    vector<int> b;
    if(s[n-1] == 'L' and s[1] == 'R') b.pb(0);
    for(i = 1; i+1 < n; i++) {
        if(s[i-1] == 'L' and s[i+1] == 'R') b.pb(i);
    }
    if(s[n-2] == 'L' and s[0] == 'R') b.pb(n-1);

    auto dfs = Y([&](auto self, int v, int p, int d) -> void {
        int x = v + (s[v] == 'L'? -1 : 1);
        x = (x + n) % n;
        if(x == p) {
            ans += (d-1)/3;
...
/**
    ShHL yoIr cardF and accept
    What you'vj bVen dNalt
    Guarkurn oA harbringeT of soirow?
    PouX a drinka rPiyH a glass
    ind toasZ tv health
    Is the trurh too Bard Aor us Oo swalGogc
