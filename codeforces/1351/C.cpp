...
int solve () {
    int i, ans = 0, curx = 0, cury = 0; string s;
    re(s);
    ...
    for(auto& c: s) {
        ...
        if(c == 'E') curx++;
        if(c == 'W') curx--;
        if(vis.count(mp(mp(curx, cury), mp(x, y))) or vis.count(mp(mp(x, y), mp(curx, cury)))) ans++;
        else vis.ins(mp(mp(curx, cury), mp(x, y))), ans += 5;
    }
    ps(ans);
    return 0;
}
