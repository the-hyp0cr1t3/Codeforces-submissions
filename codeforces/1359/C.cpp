...
int solve () {
    int i, T, h, c, o = 0, ans = DESPACITO; db RES = DESPACITO;
    re(h, c, T);

    auto check = [&] (int x) {
        db res = (db(x*h) + db((x-o)*c))/(db(2*x-o));
        res = abs(res-db(T));
        return res;
    };
    
    auto ternarySearch = [&] (int low, int high) {
        while(high-low >= 4) {
            int m1 = (2*low + high) / 3;
            int m2 = (low + 2*high) / 3;
        ...
        for(int j = low; j <= high; j++) {
            auto dd = check(j);
            if(dd < RES) RES = dd, ans = 2*j-o;
            else if(dd == RES) chmin(ans, 2*j-o);
        }
    };
