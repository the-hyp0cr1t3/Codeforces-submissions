...
namespace output {
    ...
    namespace trace {
        ...
        #define tran(x, n) __fn(#x, x, n) // TO DO~ variadic multidimensional
        ...
        Tp void __fn(const char* name, const T& x, int n) { for(int i = 0; i < n; i++) pr(name), __f(name, x[i], i); }
    }
}
...
const int N = 3e3 + 5;
int n, a[N], b[N], c[N], dp[2][2];
int32_t main() {
    IOS;
    int i, j, k, l, r, cur = 0;
    re(n); rea(a, n, b, n, c, n);
    dp[cur^1][0] = -INF;
    for(i = 0; i < n; i++, cur^=1) {
        dp[cur][0] = max(dp[cur^1][1] + a[i], dp[cur^1][0] + b[i]);
        dp[cur][1] = max(dp[cur^1][1] + b[i], dp[cur^1][0] + c[i]);
    }
    ps(dp[cur^1][0]);
    return 0;
}