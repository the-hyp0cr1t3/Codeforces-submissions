...
auto cmp = [] (const pii A, const pii B) {
    ...
    return A.wt < B.wt;                         // comparing
};

ll primsMST(int root) {
    ...
    while(!s.empty()) {
        ...
        if(~parent[edge.node]) {
            int u = edge.node;
            int v = parent[edge.node];
            int w = edge.wt;
            tree[u].ins({v, w});
            tree[v].ins({u, w});
        }
        for(auto x : g[edge.node]) {
            if(vis[x.node])
                continue;
            if(inedgewt[x.node] > x.wt) {               // comparing
                s.erase({x.node, inedgewt[x.node]});
                inedgewt[x.node] = x.wt;
                s.ins({x.node, inedgewt[x.node]});
                parent[x.node] = edge.node;
            }
        }
    }
    return cost;
...
void dfs(int v, int p) {
    anc[0][v] = p;
    auto it = tree[v].lb({p, -INF});
    if(v) dp[0][v] = it->wt;
    for(int k = 1; k < LG; k++) {
        if(~anc[k-1][v]) {
            anc[k][v] = anc[k-1][anc[k-1][v]];
            dp[k][v] = max(dp[k-1][v], dp[k-1][anc[k-1][v]]);
        }
        else
            anc[k][v] = dp[k][v] = -1;
    }
...
int LCA(int u, int v) {
    ...
    for(int i = LG-1; ~i; --i) 
        if(d[u]-(1<<i) >= d[v])
            chmax(maxedge, dp[i][u]), u = anc[i][u];
    if(u == v)
        return maxedge;
    for(int i = LG-1; ~i; --i)
        if(anc[i][u]^anc[i][v])
            chmax(maxedge, dp[i][u], dp[i][v]), u = anc[i][u], v = anc[i][v];
    chmax(maxedge, dp[0][u], dp[0][v]);
    return maxedge;
}
...
int32_t main() {
    ...
    dfs(0, -1);
    for(auto& x: edges) {
        int u = x.node.ff, v = x.node.ss, w = x.wt;
        if(tree[u].count({v, w})) {
            ps(ans);
            continue;
        }
        int maxedge = LCA(u, v);
        ll res = ans - maxedge + w;
        ps(res);
    }
    return 0;
}