...
int32_t main() {
    IOS;
    int i, n, k, sum = 0, b, mxb = 0, ans = 0, A, B;
    re(n, k);
    for(i = 1; i <= n; i++) 
        re(b), pref[i] = b + pref[i-1];
    for(i = n-2*k+1; i; i--) {
        if (pref[i+2*k-1] - pref[i+k-1] >= mxb) 
            b = i+k, mxb = pref[i+2*k-1]-pref[i+k-1];
        if (pref[i+k-1]-pref[i-1] + mxb >= ans)
            ans = pref[i+k-1]-pref[i-1] + mxb, A = i, B = b;
    }
    ps(A, B);
    return 0;
}
