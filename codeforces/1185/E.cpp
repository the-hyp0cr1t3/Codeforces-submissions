...
bool work(char c) {
    ...
    if (sz(rows) == 1) {
        ...
        for(i = fst; i <= lst; i++)
            ans[*rows.begin()][i] = c;
        vector<int> res{*rows.begin(), fst, *rows.begin(), lst};
        steps.pb(res);
    }
    else {
        int fst = *rows.begin();
        ...
        for(i = fst; i <= lst; i++)
            ans[i][*cols.begin()] = c;
        vector<int> res{fst, *cols.begin(), lst, *cols.begin()};
        steps.pb(res);
    }
    return 1;
}
...
int solve () {
    ...
    for(i = 1; i <= n; i++) 
        for(j = 1; j <= m; j++) 
            re(a[i][j]), chmax(lastchar, a[i][j]);

    // ps("y1");

    for(char cur = 'a'; cur <= lastchar; cur++)
        if (!work(cur)) return ps("NO"), 0;

    // ps("y2");

    for(i = 1; i <= n; i++) 
        for(j = 1; j <= m; j++) 
            if (a[i][j] != ans[i][j]) return ps("NO"), 0;
