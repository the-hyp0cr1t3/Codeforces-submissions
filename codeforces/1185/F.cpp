...
const int N = 1e5 + 5;
const int M = (1<<9);
int friends[N], id[M][2], dp[M], cost[N];
pii choice[M];
int32_t main() {
    IOS;
    int i, j, n, m, best = -1, bestcost = DESPACITO;
    pii ans;
    re(n, m);
    for(i = 0; i < M; i++) 
        dp[i] = DESPACITO;
    ...
    for(i = 1; i <= m; i++) {
        ...
        if(!id[mask][0]) 
            ...
            if(cost[id[mask][0]] > cost[id[mask][1]]) 
                swap(id[mask][0], id[mask][1]);
            if(cost[id[mask][1]] > cost[i]) 
                id[mask][1] = i;
        }
    ...
    for(int mask1 = 0; mask1 < M; mask1++) {
        for(i = 0; i < 2; i++) {
            if(!id[mask1][i]) continue;
            for(int mask2 = mask1; mask2 < M; mask2++) {
                for(j = 0; j < 2; j++) {
                    if(!id[mask2][j] or id[mask2][j] == id[mask1][i]) continue;
                    if(cost[id[mask1][i]] + cost[id[mask2][j]] < dp[mask1 | mask2]) {
                        dp[mask1 | mask2] = cost[id[mask1][i]] + cost[id[mask2][j]];
                        choice[mask1 | mask2] = {id[mask1][i], id[mask2][j]};
                    }
                }
            }
        }
    ...
    for(int mask = 0; mask < M; mask++) {
        if(dp[mask] == DESPACITO) continue;
        int cnt = 0;
        for(i = 1; i <= n; i++)
            cnt += ((mask | friends[i]) == mask);
        if(cnt == best and dp[mask] < bestcost)
            bestcost = dp[mask], ans = choice[mask];
        if(cnt > best)
            best = cnt, bestcost = dp[mask], ans = choice[mask];
    }
