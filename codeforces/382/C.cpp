...
int32_t main() {
    ...
    re(n); rea(a, n);
    sort(a, a+n);
    if (n == 1) return ps(-1), 0;
    if (n == 2) {
        int d = a[1]-a[0];
        if (d&1) {
            ans.ins(a[0]-d); ans.ins(a[1]+d); 
            return ps(sz(ans), e, ans), 0;
        }
        else {
            ans.ins(a[0]-d); ans.ins(a[1]+d); ans.ins(a[0]+d/2);
            return ps(sz(ans), e, ans), 0;
        }
    }
    for(i = 1; i < n; i++) 
    ...
    if (sz(diffs) > 2) 
        return ps(0), 0;
    if (sz(diffs) == 1) {
        ans.ins(a[0]-diffs.begin()->ff); ans.ins(a[n-1]+diffs.begin()->ff); 
        return ps(sz(ans), e, ans), 0;
    }
    pii d = *diffs.rbegin();
    if (d != make_pair(2*diffs.begin()->ff, 1))
        return ps(0), 0;
    for(i = 1; i < n; i++) 
        if (a[i]-a[i-1] == d.ff) return ps(1, e, a[i-1] + d.ff/2), 0;
    return 0;
}
