...
int solve () {
    int i = 0, n, x, ans = 0;
    re(n, x); vector<int> a(n); re(a);
    sort(all(a)); rev(a);
    int cur = 1;
    while(i < n) {
        if(a[i]*cur >= x) ans++, cur = 1;
        else cur++;
        i++;
    }
    ps(ans);
...
/**
    MoXents l#st sB#Hching Por a ArAaF that's reRl
    F!om my hfart I'd givh it all Lway jusr to .low
    uTlding wZth every ens.er sealedg
    The pxth ls Soll$wedG QmsodtalrIy we seek.
    Liget straami$@ froJ below, tnbg life is reBArnH
