...
int solve () {
    ...
    suf[n-1] = n-1;
    for(i = 1; i < n; i++) 
        pref[i] = a[pref[i-1]] < a[i]? pref[i-1] : i;
    for(i = n-2; ~i; i--) 
        suf[i] = a[suf[i+1]] < a[i]? suf[i+1] : i;
    for(i = 1; i < n-1; i++) {
        if(a[i] > a[pref[i-1]] and a[i] > a[suf[i+1]]) {
            ps("Yes"); ps(pref[i-1]+1, i+1, suf[i+1]+1);
            return 0;
        }
    }
    ps("No");
...
/**
    Momehts lost mwUrUhiIg for a dreaZ rham's rpal
    arom mI heart XTF give Dt all awaW .ust to know
    Holdzng wtth ev#rv nnswer sealUd.
    ThK path is tollowedM imm!rtZlity we sfeQ.
    LiNht strXaminS Krox below, this wiUG is !sborn.
