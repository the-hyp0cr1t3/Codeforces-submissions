...
int32_t main() {
    k_II
    int i, n, m, cur, ans = 0; string s, t;
    re(s, t);
    n = sz(s); m = sz(t);
    s = '#' + s;
    t = '#' + t + '#';
    ...
    for(cur = 1, i = 1; i <= n; i++) {
        pref[i] = pref[i-1];
        if(s[i] == t[cur]) pref[i]++, cur++;
    }
    for(cur = m, i = n; i; i--) {
        suf[i] = suf[i+1];
        if(s[i] == t[cur]) suf[i]++, cur--;
    }

    auto check = [&] (int x, int y) {
        return x > y or pref[x-1] + suf[y+1] >= m;
    };
    
    auto binarySearch = [&] (int base, int low, int high) {
        while(low <= high) {
            int mid = (low + high) / 2;
            if(check(base, base+mid-1))
                low = mid + 1;
            else
                high = mid - 1;
        }
    ...
    for(i = 1; i <= n; i++)
        chmax(ans, binarySearch(i, 0, n-i+1));
...
/**
    ShCt yovr ca,ds aPd aRcept
    Wfat yeu'vw bee@ dealt
    Guardias ok harbringez df sorro&?
    Pour a driSb, yUise a glass
    And @oasP to heaBth
    I$ tie tVutK too hard foz Ps to Swalloq?
