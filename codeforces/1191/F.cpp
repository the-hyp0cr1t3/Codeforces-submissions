...
#define Treetype int
using ordered_set = tree<Treetype, null_type, less<Treetype>, rb_tree_tag, tree_order_statistics_node_update>;
...
int32_t main() {
    IOS;
    int i, n, z = 0, prvy = -INF;
    ll ans = 0;
    re(n); rea(p, n);
    sort(p, p+n, [](pii A, pii B) { if(A.y == B.y) return A.x < B.x; return A.y > B.y; });
    vector<int> no{-1};
    for(i = 0; i < n; i++) {
        s.ins(p[i].x);
        if (p[i].y == prvy)
            ...
            no.pb(z);
            ans += 1ll*z*(z+1)/2;
            for(int j = 1; j < sz(no); j++) {
                ll d = no[j]-no[j-1]-1;
                ans -= d*(d+1)/2;
            }
            no.clear();
            no.pb(-1);
            int sook = (int)s.ook(p[i].x);
            auto it = lb(all(no), (int)sook);
            if (it != no.end() and *it == p[i].x) {}
            else no.pb(s.ook(p[i].x));
            prvy = p[i].y;
        }
        z = sz(s);
    }
