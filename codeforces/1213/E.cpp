...
const int N = 2e5 + 5;
set <string> same{"aa", "bb", "cc"};
set <string> perms{"abc", "acb", "bac", "bca", "cab", "cba"};
int32_t main() {
    IOS;
    int i, n; string s, t;
    re(n, s, t);
    ps("YES");
    if(same.count(s) and same.count(t)) {
        for(i = 0; i < n; i++)  
            pr("abc");
        return 0;
    }
    if(same.count(t)) swap(s, t);
    if(same.count(s)) {
        rev(t);
        string ans = t;
        set <char> ss{'a','b', 'c'};
    ...
    auto check = [&] (string v) {
        for(i = 0; i < 3; i++) {
            if(v[i] == s[0] and v[(i+1)%3] == s[1]) return false;
            if(v[i] == t[0] and v[(i+1)%3] == t[1]) return false;
        }
        return true;
    };
    for (auto& x: perms) {
