...
struct sparse {
    ...
    sparse(function<int(int, int)> Cmp) : cmp(Cmp) {}
    ...
    int query(int l, int r) {
        int k = _log(r-l+1);
        r = r-(1<<k)+1;
        return cmp(table[k][l], table[k][r]);
    }
};

sparse G([](int A, int B) { return __gcd(A, B); });
sparse M([](int A, int B) { return min(A, B); });
...
int32_t main() {
    ...
    auto check = [&] (int x) {
        for(i = 0; i+x <= n; i++) 
            if(G.query(i, i+x-1) == M.query(i, i+x-1)) return true;
        return false;
    };
    ...
    auto x = binarySearch(1, n);

    vector<int> ans;
    for(i = 0; i+x <= n; i++) 
        if(G.query(i, i+x-1) == M.query(i, i+x-1)) 
            ans.pb(i+1);
    ps(sz(ans), x-1, e, ans);
    return 0;
}