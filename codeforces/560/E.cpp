...
namespace output {
    ...
    Tp2 void pna(T1&& a, T2 t) { for(int i = 0; i < t; i++) pr(a[i]), ps(); }
...
const int M = 2e3 + 5;
pii p[M];
num fact[N], factinv[N], inv[N], f[M];
num nCr (int n, int r) {
    if (r > n) return num(0);
...
int32_t main() {
    ...
    num ans;
    for(i = 0; i < k; i++) {
        f[i] = nCr(p[i].ff+p[i].ss-2, p[i].ff-1);
        for(j = 0; j < i; j++)
            if (p[j].ss <= p[i].ss) 
                f[i] -= f[j] * nCr(p[i].ff+p[i].ss-(p[j].ff+p[j].ss), p[i].ff-p[j].ff);
        ans += f[i] * nCr(n+m-(p[i].ff+p[i].ss), n-p[i].ff);;
    }
    ans = nCr(n+m-2, n-1) - ans;
    ps(ans);
    return 0;
}
