...
int32_t main() {
    ...
    rev(edges);
    for(auto& [w, v]: edges) {
        if(same(v.ff, v.ss)) continue;
        ans += 2 * w * size_[getpar(v.ff)] * size_[getpar(v.ss)];
        merge(v.ff, v.ss);
    }
    db fans = (db)ans/db(n*(n-1));
    cout << fixed << setprecision(8) << fans;
    return 0;
}