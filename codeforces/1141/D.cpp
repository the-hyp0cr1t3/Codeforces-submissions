...
int32_t main() {
    ...
    for(i = 0; i < 26; i++) {
        for(j = 0; j < min(sz(l[i]), sz(r[i])); j++) 
            ans.pb(l[i][j], r[i][j]);
        for(k = j; k < sz(l[i]); k++)
            if(sz(storer))
                ans.pb(l[i][k], *storer.begin()), storer.erase(storer.begin());
        for(k = j; k < sz(r[i]); k++)
            if(sz(storel))
                ans.pb(*storel.begin(), r[i][k]), storel.erase(storel.begin());
    }
    while(sz(storel) and sz(storer)) {
        ans.pb(*storel.begin(), *storer.begin());
        storer.erase(storer.begin());
        storel.erase(storel.begin());
    }
    ps(sz(ans));
    pn(ans);
    return 0;
