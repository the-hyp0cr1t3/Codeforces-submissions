...
int32_t main() {
    ...
    if(sum <= 0) {
        for(i = 1; i <= n; i++) 
            if(pref[i] >= H) return ps(i), 0;
        return ps(-1), 0;
    }
    auto check = [&] (int x) {
        int h = H;
        if(x and sum and h/x < sum or h/sum < x)
            return true;
        h -= x*sum;
        if(h <= 0)
            return true;
        for(i = 1; i <= n; i++) 
            if(pref[i] >= h) return true;
        return false;
    };
    ...
    auto x = binarySearch(0, N);
    H -= x*sum;
    if(H <= 0) return ps(x*n), 0;
    for(i = 1; i <= n; i++) 
        if(pref[i] >= H) return ps(x*n+i), 0;
    assert(false);
    return 0;
}