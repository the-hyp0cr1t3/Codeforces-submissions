...
const int N = 2e5 + 5;
const vector<int> v{4 , 8, 15, 16, 23, 42};
map<int,pii> m;
int ans[7];
int32_t main() {
    IOS;
    ...
    auto x = m[query(1, 2)];
    auto y = m[query(1, 3)];
    ans[1] = x.ff == y.ff or x.ff == y.ss? x.ff : x.ss;
    ans[2] = ans[1]^x.ff^x.ss;
    ans[3] = ans[1]^y.ff^y.ss;
    x = m[query(4, 5)];
    y = m[query(4, 6)];
    ans[4] = x.ff == y.ff or x.ff == y.ss? x.ff : x.ss;
    ans[5] = ans[4]^x.ff^x.ss;
    ans[6] = ans[4]^y.ff^y.ss;
    pw('!');
    for(i = 1; i <= 6; i++) 
        cout << ans[i] << " \n"[i == 6];
    return 0;
