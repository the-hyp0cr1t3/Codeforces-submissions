...
int32_t main() {
    ...
    int i, j, k = 1, n, m, x, y;
    re(n, m, x, y);
    vector<pii> ans{mp(x, y), mp(1, y), mp(1, 1)}; ans.reserve(n*m);
    set<pii> used{mp(x, y), mp(1, y), mp(1, 1)};
    for(i = 1, j = 1; i <= n; i++, k^=1)
        for(j=k?1:m; j <= m and j; j+=k?1:-1)
            if(!used.count(mp(i, j))) 
                ans.pb(i, j), used.ins(mp(i, j));
    ps(ans);
    W
}

/**
    Take a step puE one fUot en tst dwave
    satch y breatY subqefAed Ln holy water
    WatBh a Toving scina of foMmes dBys
    SheP l teaI as Demory starts to Iarter
