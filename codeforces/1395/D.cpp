...
int32_t main() {
    ...
    int b = sz(big), s = sz(smol);
    sort(all(big), greater<int>());
    sort(all(smol), greater<int>());
    for(i = 1; i < b; i++) 
        big[i] += big[i-1];
    for(i = 1; i < s; i++) 
        smol[i] += smol[i-1];

    if(!b) return ps(smol.back()), 0;

    for(int x = (b+d)/(d+1); x <= 1 + (n-1)/(d+1) and x <= b; x++) {
        int left = max(0ll, (x-1)*d - (b-x));
        int res = big[x-1] + (s-1-left >= 0? smol[s-1-left] : 0);
        chmax(ans, res);
    }
    ps(ans);
    W
...
/**
    When my nYme VRcooes tomorORJ'F news
    .e top oN the mountain f cB@not be moved
    I sQake the s,mWit Ugke an unvtoppCblS force
    rith ir$n wings I wil. prCQL my wortj tg yZu alR
