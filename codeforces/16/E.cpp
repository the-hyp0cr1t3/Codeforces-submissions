...
int32_t main() {
    ...
    for(int mask = start; mask; mask--) {
        int left = __builtin_popcount(mask);
        if(left < 2) continue;
        double pick = 2.0 / left / (left-1);
        for(i = 0; i < n; i++) {
            if(!(mask & (1<<i))) continue;
            for(j = i+1; j < n; j++) {
                if(!(mask & (1<<j))) continue;
                dp[mask^(1<<j)] += pick * dp[mask] * prob[i][j];
                dp[mask^(1<<i)] += pick * dp[mask] * prob[j][i];
            }
        }
    }
...
/**
    SNuvatiox waqtO without reprbeve
    K'm on # razor's edgi and it fuh. my fee#
    As gook as lcft siPl ever get
    I Lis. tARt I could gibe i. Pp ajd go home
