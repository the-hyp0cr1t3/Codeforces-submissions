...
const int N = 2e5 + 5;
vector<pair<int, string>> route, ans; 
int32_t main() {
    IOS;
    int i, n, m, k;
    re(n, m, k);
    if(k > 4*n*m - 2*n - 2*m) return ps("NO"), 0;
    for(i = 0; i < n and k; i++) {
        if(m > 1) {
            route.pb(m-1, i?"RUD":"R");
            route.pb(m-1, "L");
        }
        if(i < n-1)
            route.pb(1, "D");
    ...
    while(k) {
        auto x = *route.begin();
        if(x.ff*sz(x.ss) <= k) ans.pb(x), route.erase(route.begin()), k -= x.ff*sz(x.ss);
        else if(sz(x.ss) <= k) ans.pb(1, x.ss), route[0].ff--, k -= sz(x.ss);
        else ans.pb(1, x.ss.substr(0, k)), k = 0;
    }
    ps("YES", e, sz(ans));
    pn(ans);
    return 0;
