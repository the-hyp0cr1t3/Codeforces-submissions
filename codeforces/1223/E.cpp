...
void dfs(int v, int p, vector<vector<pii>>& g, vector<vector<int>>& dp) {
    int i; vector<pii> s;
    for(auto [x, w]: g[v]) {
        if(x == p) continue;
        dfs(x, v, g, dp);
        s.pb(dp[x][1]+w-dp[x][0], dp[x][0]);
    }
    sort(all(s)); rev(s);
    for(i = 0; i < min(sz(s), k-1); i++) {
        dp[v][1] += max(0ll, s[i].ff) + s[i].ss;
        dp[v][0] += max(0ll, s[i].ff) + s[i].ss;
    }
    if(i < sz(s))
        dp[v][0] += max(0ll, s[i].ff);
    for(; i < sz(s); i++) {
        dp[v][1] += s[i].ss;
        dp[v][0] += s[i].ss;
    }
