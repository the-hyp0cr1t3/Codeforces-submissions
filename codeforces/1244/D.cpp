...
const int N = 1e5 + 5;
int n, c[3][N], root, deg[N], res = 0, best = DESPACITO, col[N];
vector<int> g[N], curperm{0, 1, 2};
vector<bool> vis;
...
void dfs(int v, int p, int k) {
    res += c[curperm[k]][v];
    for (auto x: g[v]) 
        if (x^p) dfs(x, v, (k+1)%3);
}

void color(int v, int p, int k) {
    col[v] = curperm[k]+1;
    for (auto x: g[v]) 
        if (x^p) color(x, v, (k+1)%3);
}
...
int32_t main() {
    ...
    for(i = 0; i < 3; i++) 
        for(j = 0; j < n; j++) 
            re(c[i][j]);
    if(!makeG()) return ps(-1), 0;
    do {
        res = 0;
        dfs(root, -1, 0);
        if(res < best) 
            best = res, color(root, -1, 0);
